...
module Auth.OAuth2.AuthorizationSpec where
...
doAuthz env = runIdentity $ processAuthorizationRequest allowedResponseTypes getClient gc createAuthorization resourceOwnerApproval createAccessToken createIdToken (return $ Just (TU "cat", True)) env now

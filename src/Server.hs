...
import           Model hiding (Email)
import           OAuth2.Authorize
import           OAuth2.ClientAuth
import           OAuth2.Token
import           OpenID.Discovery (mkOpenIDConfiguration)
import           OpenID.IdToken
import           OpenID.Registration
import           OpenID.UserInfo
import qualified Server.BlazeUI as UI
...
authServer :: (Subject s)
    => Config IO s
    -> (Client -> [Scope] -> Int64 -> Html)
    -> Handler (Maybe (s, Bool))
    -> Handler ()
    -> R.PathMap (Handler ())
authServer config@Config {..} approvalPage authenticatedUser authenticateUser =
  ...
  where
    ...
    authorizationHandler = do
      ...
      where
        ...
        resourceOwnerApproval :: Subject s => s -> Model.Client -> [Scope] -> POSIXTime -> Handler [Scope]

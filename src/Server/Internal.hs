...
import qualified Server.Session as S
...
routerToApp :: S.LoadSession->R.PathMap (Handler ())-> Application
routerToApp loadSesh router req respond =
    let nf404 _ respond' = respond' (responseLBS notFound404 [] "Not Found")
    in  routerToMiddleware loadSesh router nf404 req respond

routerToMiddleware :: S.LoadSession->R.PathMap (Handler ())-> Middleware
routerToMiddleware loadSesh router app req respond = case R.match router (pathInfo req) of
    [h] -> do
  ...
  where
    ...
    execHandler rd h = do
        ...
        return $ case result of
            ...
            Left (Redirect url)   -> redirectFull url hdrs
